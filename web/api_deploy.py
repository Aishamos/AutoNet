...
TEMPLATE_FIELDS = {
    "acl.j2": [
        {"name": "acl_number", "label": "ACL 编号 (如 3000)", "type": "number"},
        {"name": "rule_id", "label": "规则 ID (如 5)", "type": "number"},
        {"name": "source_ip", "label": "源 IP (如 192.168.1.0)", "type": "text"},
        {"name": "source_wildcard", "label": "通配符掩码 (如 0.0.0.255)", "type": "text"}
    ],
    "interface.j2": [
        {"name": "interface_name", "label": "接口名 (如 GigabitEthernet0/0/1)", "type": "text"},
        {"name": "ip_address", "label": "IP 地址 (如 10.1.1.1)", "type": "text"},
        {"name": "mask", "label": "子网掩码 (如 255.255.255.0)", "type": "text"}
    ],
    "vlan.j2": [
        {"name": "vlan_id", "label": "VLAN ID (如 10)", "type": "number"},
        {"name": "vlan_description", "label": "VLAN 描述 (可选)", "type": "text"}
    ]
}
...
def run_deploy():
    ...
    try:
        ...
        if res.returncode == 0:
            return jsonify({"status": "success", "message": f"✅ 配置成功下发至 [{target}]", "log": full_log, "results": device_results})
        else:
            # 判断是全部失败还是部分失败
            if device_results and any(device_results.values()):
                status = "partial"
                failed = [h for h, ok in device_results.items() if not ok]
                msg = f"⚠️ 部分设备下发失败: {', '.join(failed)}"
            else:
                status = "error"
                msg = "下发失败，请查看日志"
            return jsonify({"status": status, "message": msg, "log": full_log, "results": device_results}), 500
